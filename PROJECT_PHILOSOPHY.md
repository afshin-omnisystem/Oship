...
Dependencies: .ai/INDEX.md, .ai/PROJECT_STATUS.md, .ai/CURRENT_CONTEXT.md
Related Files: README.md, docs/INDEX.md, docs/MASTER_CONTEXT/ENTERPRISE_ARCHITECTURE_CONTEXT.md
...
stateDiagram-v2
    [*] --> Idle: No active task
    Idle --> GoalSetting: Human defines objective
    GoalSetting --> ContextGathering: AI requests context
    ContextGathering --> Researching: AI explores knowledge
    Researching --> Planning: AI drafts approach
    Planning --> HumanReview: AI requests approval
    HumanReview --> Planning: Revisions requested
    HumanReview --> Executing: Approved
    Executing --> Documenting: Implementation complete
    Documenting --> HumanReview: Final approval
    HumanReview --> Delivered: Accepted
    Delivered --> Idle: Task complete
```
...
graph TB
    ...
    D3 --> I3[Implementation]
    I1 --> T1[Tests]
    I2 --> T2[Tests]
    I3 --> T3[Tests]
    Q --> G1[Guides]
    S --> G2[Guides]
    T1 --> DOC1[Documentation]
    T2 --> DOC2[Documentation]
    T3 --> DOC3[Documentation]
    DOC1 --> ADR1[ADR]
    DOC2 --> ADR2[ADR]
    DOC3 --> ADR3[ADR]
    ADR1 --> A
...
sequenceDiagram
    ...
    participant KNOWLEDGE as Knowledge Kernel
    participant FILESYSTEM as File System
    participant TASK as Task Manager
    
    AI->>BOOT: Initialize
    BOOT->>CONTEXT: Load Current Context
    CONTEXT->>CONTEXT: Read PROJECT_STATUS
    CONTEXT->>CONTEXT: Read CURRENT_CONTEXT
    CONTEXT->>CONTEXT: Read NEXT_ACTION
    CONTEXT->>KNOWLEDGE: Load Philosophy
    KNOWLEDGE->>FILESYSTEM: Verify Structure
    FILESYSTEM->>TASK: Queue Tasks
    TASK->>AI: Ready to Execute
```
...
```mermaid
graph TD
    subgraph "Enterprise Engineering Principles"
        A[Architecture Principles]
        B[Quality Principles]
        C[Security Principles]
        D[Scalability Principles]
        E[Performance Principles]
        F[Maintainability Principles]
    end
...
flowchart LR
    subgraph "Quality Gates"
        G1[G1: Syntax] --> G2[G2: Type Check]
        G2 --> G3[G3: Unit Tests]
        G3 --> G4[G4: Integration Tests]
        G4 --> G5[G5: Security Scan]
        G5 --> G6[G6: Code Review]
        G6 --> G7[G7: Documentation Check]
        G7 --> G8[G8: Merge]
    end
...
```mermaid
flowchart TD
    A[Refactor] --> B{Has Tests?}
    B -->|No| C[Write Tests First]
    C --> B
    B -->|Yes| D{Make Small Changes}
    D -->|No| E[Break Down Changes]
    E --> D
    D -->|Yes| F[Run Tests]
    F --> G{Tests Pass?}
    G -->|No| H[Fix Issues]
...
stateDiagram-v2
    [*] --> Foundation: Phase 0
    Foundation --> Growth: Phase A
    Growth --> Expansion: Phase B
    Expansion --> Maturity: Phase C
    Maturity --> Stability: Phase D
    Stability --> Production: Phase E
    Production --> Evolution: Phase F
    Evolution --> [*]: End of Life
    
    note right of Production: v1.0.0 GA
    note right of Evolution: Continuous improvement
```
...
```mermaid
flowchart TD
    A[Plan Release] --> B[Feature Freeze]
    B --> C[Code Freeze]
    C --> D[Testing]
    D --> E{All Tests Pass?}
    E -->|No| F[Fix Issues]
    F --> D
    E -->|Yes| G[Release Candidate]
    G --> H[Burn-in Testing]
    H --> I{Stable?}
    I -->|No| J[Patch RC]
    J --> H
    I -->|Yes| K[Final Review]
    K --> L[Publish Release]
...
flowchart LR
    ...
    B --> C{Evaluation}
    C -->|Incomplete| D[Refine Prompt]
    C -->|Incorrect| D
    C -->|Unclear| D
    D --> B
    C -->|Satisfactory| E[Document Prompt]
...
flowchart TD
    ...
    H --> I{Review}
    I -->|Changes Needed| J[Make Changes]
    J --> F
    I -->|Approved| K[Squash and Merge]
    K --> L[Delete Branch]
    L --> M[Sync with Upstream]
```
...
```mermaid
flowchart TD
    A[Project Success] --> B[Technical Success]
...
```mermaid
flowchart TD
    A[Failure Detected] --> B{Is it Critical?}
    B -->|Yes| C[Stop the bleeding]
    B -->|No| D[Log and triage]
    C --> E[Root Cause Analysis]
    D --> E
    E --> F[Fix implemented]
    F --> G[Lessons learned]
    G --> H[Update processes]
    H --> I[Communicate]
```
...
flowchart LR
    subgraph "GOLDEN RULES"
        A[Documentation Drives Code]
        B[Change Updates Knowledge]
        C[Repository Never Forgets]
        D[Decision Is Recorded]
        E[AI Reads Context First]
        F[Nothing Is Final]
        G[Everything Is Versioned]
        H[Architecture Precedes Code]
        I[Improvement Becomes Knowledge]
        J[Documents Help AI]
    end
...
```mermaid
flowchart TD
    A[Violation Detected] --> B{Automated or Manual?}
    B -->|Automated| C[Block Action]
    B -->|Manual| D[Review by Architecture]
    C --> E[Alert Owner]
    D --> F{Valid Violation?}
    F -->|No| G[False Positive]
    F -->|Yes| H[Escalate]
    E --> I[Document Violation]
    I --> J[Require Fix]
    J --> K{Fixed?}
    K -->|No| L[Enforce Consequences]
    K -->|Yes| M[Close Violation]
    G --> N[Update Rules]
    H --> I
```
...
```mermaid
flowchart TD
    START[AI Agent Initialized] --> A[Read .ai/INDEX.md]
    A --> B[Read PROJECT_PHILOSOPHY.md]
    B --> C[Read PROJECT_STATUS.md]
    C --> D[Read CURRENT_CONTEXT.md]
    D --> E[Read NEXT_ACTION.md]
    E --> F[Read SESSION_MEMORY.md]
    F --> G[Read README.md]
    G --> H[Read docs/INDEX.md]
    H --> I[Read Domain Docs]
    I --> J{Context Complete?}
    J -->|No| K[Read More Context]
    K --> J
    J -->|Yes| L[Begin Task]
...
```mermaid
flowchart TD
    A[External Trigger] --> B[Market Change]
    A --> C[Technology Shift]
    A --> D[User Feedback]
    
    E[Internal Trigger] --> F[Tech Debt]
    E --> G[Scale Limit]
    E --> H[Quality Issue]
    
    B --> I[Evolution Required]
    C --> I
    D --> I
    F --> I
    G --> I
    ...
    I --> J{Type of Evolution?}
    J -->|Small| K[Incremental Update]
    J -->|Medium| L[Refactoring]
    J -->|Large| M[Architectural Change]
    J -->|Major| N[Paradigm Shift]
```
