...
## Sprint 038 — Unified Predictive Opportunity Intelligence & Evidence-Bound Scoring Engine Glossary

- **Opportunity Candidate** — a new opportunity submitted for intelligence assessment: identity, receivedAt, domain, class, strategy, canonically sorted unique venues, domain-scoped venue legs (AFIS BUY/SELL without odds or betting identity; ABL BACK/LAY with optional decimal odds > 1 and mandatory marketId+selectionId) and market condition indices (theoretical edge, spread, liquidity/imbalance/volatility/execution indices each optional in [0,1]).
- **Rejection Code** — one of thirteen fail-closed rejection reasons (MISSING_IDENTITY, UNKNOWN_DOMAIN, UNKNOWN_CLASS, CLASS_DOMAIN_MISMATCH, MISSING_STRATEGY, INVALID_STRATEGY_REFERENCE, INVALID_VENUE_REFERENCE, INVALID_NUMERICAL_VALUE, AMBIGUOUS_SEMANTIC_MAPPING, MALFORMED_CANDIDATE, MISSING_ABL_IDENTITY, INVALID_ODDS, NON_CANONICAL_ORDER). Strategy and venue references must exist in the consumed learning history, in the candidate's domain; nothing is silently repaired.
- **Similarity Assessment** — deterministic explicit-feature similarity between a candidate and each same-domain historical observation: classMatch, strategyMatch, venue Jaccard overlap, edge proximity (candidate edge vs theoretical net) and execution proximity, renormalized over available dimensions, floored and top-K capped with canonical tie-breaks (score desc → observationId asc). Cross-domain observations are never in the pool.
- **Feature Profile** — domain-scoped Sprint 037 feature summaries for the candidate's class, strategy and venues with sample sizes, classifications, mean preservation, trend, stability and evidence state; absent subjects are honest INSUFFICIENT summaries, never invented.
- **Regime Match** — the candidate's current condition indices matched against historical regime dimensions by explicit proximity; the nearest era wins deterministically; UNAVAILABLE (never a guess) when no dimension is measurable on both sides.
- **Strategy / Venue Fit** — deterministic classification→sub-score maps: CONSISTENT_OUTPERFORMER 1.0, IMPROVING 0.8, STABLE 0.6, DETERIORATING 0.3, HIGH_THEORETICAL_LOW_REALIZATION 0.2, CONSISTENT_UNDERPERFORMER 0.1; CONSISTENTLY_STRONG 1, IMPROVING 0.7, DETERIORATING 0.3, CONSISTENTLY_WEAK 0.2; null when history is insufficient.
- **Leakage Risk Assessment** — apparent quality (mean theoretical net), realized quality (mean realized net), leakage burden (mean leakage) and leakage-adjusted quality = realized + leakage: the leakage penalty is counted exactly once (added back, never subtracted twice). Leakage share is the burden over apparent value clamped to [0, cap]; recurring class components come from Sprint 037 leakage learning.
- **Evidence Profile** — source, evidence count, sample adequacy (SUFFICIENT/LIMITED/INSUFFICIENT), freshness (stale window), consistency (preservation dispersion band), completeness (share of carried analytical inputs), conflicts (contradictory evidence states or CONTRADICTORY strategy stability), domain compatibility and comparability, all rolled into an explicit confidence state.
- **Evidence Confidence** — STRONG / MODERATE / WEAK / INSUFFICIENT / CONFLICTED / STALE / NOT_COMPARABLE. Heterogeneous-but-uncontradicted evidence is WEAK (it still scores, penalized); conflicts are hard contradictions; missing evidence is never negative evidence — it blocks scoring instead.
- **Stability Integration** — Sprint 037 stability and drift classifications mapped to STABLE/UNSTABLE/IMPROVING/DETERIORATING/REGIME_SENSITIVE/INSUFFICIENT_HISTORY with an explicit factor in [0,1]. Stability affects interpretation and the score decomposition openly; it never silently overrides another signal.
- **Historical Outcome Distribution** — descriptive statistics of the similar cohort only: outcome counts, positive/neutral/negative realized-net bands (fixed at zero, measured nets only), preservation median/quartiles/min/max/dispersion, mean realized net and realization quality, all under the exact distribution disclaimer — never a probability or forecast.
- **Evidence-Bound Score** — an eleven-dimension analytical index (historicalPreservation, realizationQuality, evidenceQuality, similarityQuality, regimeFit, strategyFit, venueFit, stabilityFactor, leakageBurden, freshnessFactor, sampleAdequacy) with config-driven auditable weights, effective weights renormalized over available dimensions, and a bitwise-exact decomposition. NULL whenever evidence confidence is INSUFFICIENT, NOT_COMPARABLE, CONFLICTED or STALE. Always carries: "This is an evidence-bound analytical index, not a probability, forecast, expected return, or guarantee."
- **Dependency Analysis** — the similar cohort grouped by era (regime), strategy and candidate-venue touch; a dependency is detected when at least two qualified groups (≥ minimum group sample) show mean-preservation spread above the configured band. All groups, spreads and determinability are exposed.
- **Classification** — the deterministic verdict with precedence NOT_COMPARABLE → INSUFFICIENT_EVIDENCE → UNKNOWN (too few contributing dimensions) → REGIME_DEPENDENT → STRATEGY_DEPENDENT → VENUE_DEPENDENT → MIXED (favorable-but-unstable conflict) → HISTORICALLY_FAVORABLE (score ≥ band) → HISTORICALLY_UNFAVORABLE (score ≤ band) → MIXED (middle band), each with reconstructible reasons.
- **Domain Ranking** — per-domain deterministic ordering (score desc → candidateId asc) with contiguous ranks; INSUFFICIENT_EVIDENCE/UNKNOWN/NOT_COMPARABLE profiles are excluded with explicit reasons; rankings never mix domains.
- **Intelligence Explanation** — the reconstructible why of a profile: dimension-by-dimension contributions with weights, strong/weak dimensions, evidence sufficiency, dependencies, leakage and stability effects, missing evidence and the classification rationale. No opaque scores, no certainty claims, no authority language (enforced by invariants).
- **Research Context** — the structured informational research input: identity, similar observation ids, evidence summary, score components, classification, dependency signals, evidence gaps and recommended research questions. Research consumes it read-only; nothing auto-mutates.
- **Decision Feedback / Outcome Reconciliation** — feedback records mirror the decision context (classification, score, evidence used); reconciliation later compares prediction against an observed result (agreement, disagreement kind, informational drift signal). Reconciliation never rewrites the profile, audit or history.
- **Opportunity Intelligence Audit** — hash-chained `oship.opportunity-intelligence.v1` event stream over exactly 13 event types (opportunity-received, candidate-rejected, similarity-evaluated, evidence-evaluated, dependencies-evaluated, score-calculated, classification-selected, profile-created, ranking-generated, explanation-generated, feedback-recorded, replay-completed, fail-closed): tampering, reordering, payload substitution, truncation and extension all fail verification.
- **Opportunity Intelligence Invariants (66)** — the hard fail-closed contract: determinism (stage rebuilds byte-identical, replay identity, canonical serialization), identity, domain isolation, AFIS/ABL semantics (sides, odds, identity), evidence states and never-negative-missing, score bounds/decomposition/weight normalization/no-magic-weights/null-on-dishonest-confidence, classification consistency, the dependency rule, leakage counted once, stability explicitness, distribution historical-only and not-probability, ranking order/coverage/exclusions, explanation reconstructibility, NO_FUTURE_CERTAINTY_CLAIMS and NO_AUTHORITY_LANGUAGE scans, research informational-only, feedback mirroring, reconciliation non-destructive, audit integrity, canonical fail-closed rejections, no silent repairs, informational-only and no execution/treasury/credential surface.

## Sprint 039 — Unified Decision Intelligence & Counterfactual Evaluation Engine Glossary

- **Decision Context** — the canonical, serializable frame of one decision run: content-derived `dctx_` identity, base candidate, domain, class, strategy, venues, engine version, configuration fingerprint and informational/associational flags.
- **Candidate Alternative** — a hypothetical variation of the base opportunity under evaluation: strategy change, venue subset, execution parameters (AFIS) or orientation/market/venue variants (ABL), expressed as an `AlternativeSpec` where explicit nulls mean "no override".
- **Baseline Alternative** — the auto-constructed no-override alternative (kind BASELINE) representing the base opportunity itself; always first, always compatible, and its alternative id is reserved against duplicate submissions.
- **Alternative Rejection Code** — one of twelve fail-closed rejection reasons (MALFORMED_ALTERNATIVE, MISSING_ALTERNATIVE_IDENTITY, UNKNOWN_ALTERNATIVE_KIND, DUPLICATE_ALTERNATIVE_ID, BASE_CANDIDATE_MISMATCH, CROSS_DOMAIN_COMPARISON, INVALID_VENUE, INVALID_SIDE_SEMANTICS, NON_CANONICAL_ORDER, EMPTY_ALTERNATIVE_VARIATION, INCOMPATIBLE_UNITS, AMBIGUOUS_SEMANTIC_MAPPING). Nothing is silently repaired or coerced.
- **Compatibility Assessment** — ten-plus deterministic checks (domain, class, identity, semantics, units, ordering) that must pass before an alternative is evaluated at all.
- **Counterfactual Candidate** — the hypothetical opportunity produced by applying an alternative's explicit overrides to the base candidate; evaluated by the real Sprint 038 opportunity intelligence; `counterfactualOnly: true`, descriptive of existing evidence only.
- **Historical Comparison** — descriptive per-pair statistics (means, medians, dispersion, preservation delta = left − right) between accepted alternatives; `descriptiveOnly` always; cross-domain pairs are never comparable.
- **Dependency Axis** — one of six per-alternative analyses (regime, strategy, venue, leakage, stability, evidence); regime/strategy/venue dependencies are detected via cohort spreads with all groups exposed and are never silently generalized away.
- **Trade-off Score** — a twelve-dimension explicit analytical index (historicalPreservation, realizationQuality, evidenceQuality, similarityQuality, regimeFit, strategyFit, venueFit, leakageBurden, stabilityPenalty, evidenceGaps, complexity, freshness) with config-driven auditable weights, availability renormalization, bitwise-exact decomposition and null scores on INSUFFICIENT/NOT_COMPARABLE/CONFLICTED confidence. Not a probability, forecast, expected return or guarantee.
- **Dominance State** — DOMINANT_BY_EVIDENCE / WEAKLY_PREFERRED / NO_DOMINANT_OPTION / INSUFFICIENT_EVIDENCE / CONFLICTED / VENUE_DEPENDENT / REGIME_DEPENDENT / STRATEGY_DEPENDENT / NOT_COMPARABLE under configurable dominant/weak/tie bands. "Dominant" means better supported by existing evidence — never guaranteed, and its reasons say so.
- **Evidence-Bound Recommendation** — the decision output: status, selected alternative (null under conflict or insufficient evidence), dependency state, trade-off breakdown and supporting/opposing evidence, always informational-only and carrying the exact disclaimer that it is not a probability, forecast, expected return or execution instruction.
- **Scenario Matrix** — alternative × evidence × regime × strategy × venue cells backed only by real observation ids; unsupported combinations are explicitly INCOMPATIBLE, never inferred.
- **Decision Explanation** — the reconstructible why of a decision: summary, per-alternative rationales (accepted and rejected), evidence for/against, axis effects, conflict conditions, acceptance decisions and boundary statements; no opaque scores, no certainty claims, no authority language.
- **Decision Research Context** — the structured informational research/strategy input with evidence gaps and prioritized questions (HIGH before MEDIUM before LOW).
- **Outcome Divergence** — the immutable reconciliation of what actually happened against the recommendation: AGREEMENT / RECOMMENDED_BUT_NEGATIVE / SPURNED_ALTERNATIVE_POSITIVE / INSUFFICIENT_RECOMMENDATION / UNDETERMINABLE plus regime/strategy/venue/leakage mismatch flags. Drift is observed and recorded, never rewritten.
- **Decision Intelligence Audit** — hash-chained `oship.decision-intelligence.v1` event stream over exactly 14 event types (context-created, alternative-added, alternative-rejected, compatibility-evaluated, counterfactual-evaluated, evidence-evaluated, tradeoff-evaluated, dominance-evaluated, recommendation-generated, explanation-generated, research-context-generated, feedback-recorded, replay-completed …); tamper, reorder, substitution, truncation and extension all fail closed.
- **Decision Intelligence Invariants (83)** — the hard fail-closed contract: determinism (triple-run and stage-rebuild byte-identity), identity/linkage, AFIS/ABL isolation (no betting semantics in AFIS, preserved BACK/LAY + odds + market/selection identity in ABL), no-execution-authority, no-future-certainty-claims, no-authority-language, audit-covers-lifecycle, spec coverage, reproducible ranking, disclaimer presence, informational flags and credential-surface absence.

## Sprint 040 — Unified Decision Governance & Strategy Handoff Engine Glossary

- **Decision Governance** — the deterministic validation layer that decides whether a decision-intelligence result may be handed to Strategy, and with which limitations; informational-only.
- **Governance Context** — the immutable canonical mirror of a Sprint 039 decision result (identity, domain, recommendation/dominance/evidence states, dependencies, leakage, stability, freshness, adequacy, comparability, research gaps, versions) that gates evaluate over.
- **Evidence Gate** — PASS / PASS_WITH_LIMITATIONS / BLOCK_INSUFFICIENT_EVIDENCE / BLOCK_CONFLICTED / BLOCK_STALE / BLOCK_NOT_COMPARABLE with deterministic precedence.
- **Safety Gate** — the semantic-safety scan rejecting probability, forecast, expected-profit, ROI, guaranteed-return and certainty claims, execution instructions and fabricated output keys; the disclaimer is verbatim and negation-aware ("support is not certainty" is legal).
- **Comparability Gate** — verifies AFIS BUY/SELL and ABL BACK/LAY semantics and rejects raw cross-domain comparison; normalized comparison requires an explicit versioned, loss-declared, policy-attested mapping onto a neutral namespace.
- **Normalization (`cross-domain.normalized.v1`)** — the only legal bridge between AFIS and ABL semantics: identity-explicit, version-matched, total side mapping (never BACK==BUY), declared semantic loss, policy attestation; never inferred.
- **Freshness States** — FRESH / AGING / STALE / UNKNOWN; STALE blocks unless the policy explicitly allows analytical-only; UNKNOWN is never silently treated as FRESH.
- **Governance Stability States** — STABLE / MODERATELY_STABLE / UNSTABLE / INSUFFICIENT; explicit trade-off information, never converted to probabilities.
- **Dependency States** — INDEPENDENT / REGIME_DEPENDENT / STRATEGY_DEPENDENT / VENUE_DEPENDENT / MULTI_DEPENDENT / UNKNOWN; preserved into the handoff package.
- **Authority Check** — the boundary scan protecting Strategy Registry, AEGIS, Treasury, Execution, Portfolio, Risk and Allocation; bypass attempts become auditable in-band blocks, never executions.
- **Handoff Classification** — HANDOFF_ALLOWED, HANDOFF_ALLOWED_WITH_LIMITATIONS, HANDOFF_REQUIRES_RESEARCH, HANDOFF_BLOCKED, HANDOFF_NOT_COMPARABLE, HANDOFF_CONFLICTED, HANDOFF_STALE, HANDOFF_INSUFFICIENT_EVIDENCE; blocked handoffs surface `recommendedAlternativeId: null` — never a silent alternative.
- **Handoff Restriction** — a machine-readable limitation code (ANALYTICAL_ONLY, NO_EXECUTION, RESEARCH_REQUIRED, LIMITED_TO_DOMAIN/VENUE/STRATEGY, REGIME_SPECIFIC, STALE_EVIDENCE_WARNING, INSUFFICIENT_SAMPLE_WARNING, STABILITY_WARNING, AGING_EVIDENCE_WARNING, NORMALIZED_COMPARISON_ONLY, LEAKAGE_WARNING) with scope and reason.
- **Strategy Handoff Package (`decision-governance.handoff.v1`)** — the deterministic immutable evidence-bound handoff artifact: identity, ranking, trade-offs, dominance, limitations, dependencies, statuses, research requirements, governance result and restrictions, source versions, disclaimer and audit identity; input to Strategy, not a strategy.
- **Strategy Input View (`decision-governance.strategy-input.v1`)** — the minimal informational surface Strategy consumes; declares `strategyDecides: true`; contains no orders, instructions, sizing or credentials.
- **Research Escalation** — RESEARCH_REQUIRED, EVIDENCE_REFRESH_REQUIRED, REGIME_/STRATEGY_/VENUE_COVERAGE_REQUIRED, COMPARABILITY_REQUIRED, LEAKAGE_INVESTIGATION_REQUIRED — routed to the existing Research Plane, never a second Research authority.
- **Governance Feedback Record** — a structured informational record (blocked, allowed-with-limitations, evidence gap, stale, conflict, dependency detected, recommendation weakened/preserved) routed to the existing Learning/Feedback architecture.
- **Governance Rejection Gallery** — the enumerated fail-closed rejection paths (22 entries) over explicit codes such as INVALID_GOVERNANCE_CONTEXT, INVALID_DECISION_RESULT, MISSING_DECISION_ID, MISSING_OPPORTUNITY_ID, MISSING_DOMAIN, UNSUPPORTED_DOMAIN, NONDETERMINISTIC_INPUT, INVALID_AFIS_SEMANTICS, INVALID_ABL_SEMANTICS, INVALID_BACK_LAY_SEMANTICS, LEAKAGE_INCONSISTENCY, STABILITY_INCONSISTENCY and INVALID_RESTRICTION — no silent fallbacks.
- **Governance Audit (`oship.decision-governance.v1`)** — the append-only hash-chained event log over 16 event types covering the full governance lifecycle; tampering, reordering, substitution, truncation, extension and foreign events fail verification.
- **Governance Invariants** — the 77 hard fail-closed checks executed on every governance run (authority non-duplication, semantic safety, domain semantics, honesty, determinism, audit integrity, immutability, strategy boundary, no credential/command propagation, blocked-never-recommends).
