...
## Sprint 038 — Unified Predictive Opportunity Intelligence & Evidence-Bound Scoring Engine (`services/market-engine/src/intelligence/opportunity/`)

**THE OPPORTUNITY INTELLIGENCE ENGINE IS AN ANALYTICAL EVIDENCE LAYER, NOT AN AUTHORITY AND NOT AN ORACLE.** It turns a validated Sprint 037 `LearningResult` (consumed read-only, invariants and replay verified) plus new opportunity candidates into a deterministic intelligence profile and an evidence-bound score for every candidate. It never claims guaranteed profit, guaranteed execution, future probability, expected return, financial certainty or betting certainty; historical outcome distributions remain explicitly historical; every output is informational and associational-only. It never mutates the Strategy Registry, Portfolio, Risk, AEGIS, Treasury, Execution or provider credentials; the chain stays Human → AETHER → OSHIP Core/SuperNova → Intelligence → Strategy → AEGIS → Treasury → Execution.

Lifecycle (one `OpportunityIntelligenceEngine.analyze()`): **New Opportunity → Historical Similarity → Learned Features → Regime Match → Strategy History → Venue History → Leakage History → Evidence Quality → Stability → Historical Outcome Distribution → Opportunity Intelligence Profile → Evidence-Bound Score → Classification → Explanation → Research / Decision Input.** The pipeline runs twice per analysis and must be byte-identical; 66 fail-closed invariants are enforced on every analysis and any failure throws.

- **Candidates & Fail-Closed Validation (`types.ts`, `candidate.ts`)** — `OpportunityCandidate` carries identity, domain, class, strategy, canonically sorted unique venues, domain-scoped legs (AFIS BUY/SELL without odds or betting identity; ABL BACK/LAY with optional decimal odds > 1 and mandatory marketId+selectionId) and market condition indices. Thirteen rejection codes (MISSING_IDENTITY … NON_CANONICAL_ORDER) reject unknown domains, unknown/mismatched classes, missing strategy, invalid strategy/venue references (references must exist in the consumed learning history, in the candidate's domain), invalid numbers, ambiguous semantic mappings (cross-domain strategy mapping, BACK on AFIS, odds on AFIS, betting identity on AFIS), invalid odds, missing ABL identity and non-canonical ordering — nothing is silently repaired.
- **Similarity (`similarity.ts`)** — deterministic explicit-feature similarity over five components (classMatch, strategyMatch, venue Jaccard overlap, edge proximity vs theoretical net, execution proximity), renormalized over available dimensions; same-domain pool only (cross-domain is never similar); similarity floor and top-K with canonical tie-breaks (score desc → observationId asc).
- **Learned Features / Regime / Strategy / Venue (`feature-profile.ts`, `regime-match.ts`, `strategy-match.ts`, `venue-match.ts`)** — domain-scoped Sprint 037 lookups with honest INSUFFICIENT summaries for absent subjects (never invented); candidate condition indices matched against historical regime dimensions by explicit proximity (nearest era wins deterministically; UNAVAILABLE when nothing measurable); classification→fit mappings (strategy: CONSISTENT_OUTPERFORMER 1 … CONSISTENT_UNDERPERFORMER 0.1; venue: CONSISTENTLY_STRONG 1 … CONSISTENTLY_WEAK 0.2; null when insufficient).
- **Leakage (`leakage-risk.ts`)** — apparent quality (mean theoretical), realized quality (mean realized), leakage burden (mean leakage) and leakage-adjusted quality = realized + leakage — **the leakage is counted exactly once, never subtracted twice**; share clamped to [0, cap]; recurring class components from Sprint 037 leakage learning.
- **Evidence (`evidence.ts`)** — source, count, sample adequacy (SUFFICIENT/LIMITED/INSUFFICIENT), freshness (30-day default window), consistency (preservation dispersion band), completeness, conflicts (contradictory evidence states or a CONTRADICTORY strategy), domain compatibility and comparability. Confidence: STRONG/MODERATE/WEAK/INSUFFICIENT/CONFLICTED/STALE/NOT_COMPARABLE — heterogeneous-but-uncontradicted evidence is WEAK (still scores, penalized); **missing evidence is never negative evidence** and blocks scoring instead.
- **Stability & Distribution (`stability.ts`, `outcome-distribution.ts`)** — Sprint 037 stability + drift mapped to STABLE/UNSTABLE/IMPROVING/DETERIORATING/REGIME_SENSITIVE/INSUFFICIENT_HISTORY with an explicit factor (affects interpretation, never silently overrides); descriptive-only distribution — counts by outcome, positive/neutral/negative realized-net bands (fixed at zero, measured nets only), preservation median/quartiles/min/max/dispersion, realization quality — all under the exact `DISTRIBUTION_DISCLAIMER` ("…explicitly historical, not a future probability or forecast").
- **Score (`score.ts`)** — eleven explicit dimensions (historicalPreservation, realizationQuality, evidenceQuality, similarityQuality, regimeFit, strategyFit, venueFit, stabilityFactor, leakageBurden = 1 − share, freshnessFactor, sampleAdequacy) with config-driven weights, per-component configured and effective weights (renormalized over available dimensions), and a bitwise-exact decomposition (score = Σ contributions). **The score is NULL whenever evidence confidence is INSUFFICIENT, NOT_COMPARABLE, CONFLICTED or STALE.** Every score carries the exact `SCORE_DISCLAIMER`: "This is an evidence-bound analytical index, not a probability, forecast, expected return, or guarantee."
- **Dependencies & Classification (`classification.ts`)** — cohort grouped by era/strategy/candidate-venue-touch; a dependency is detected when ≥ 2 qualified groups (≥ min group sample) show mean-preservation spread > band, with all groups and spreads exposed. Classification precedence: NOT_COMPARABLE → INSUFFICIENT_EVIDENCE → UNKNOWN (too few contributing dimensions) → REGIME_DEPENDENT → STRATEGY_DEPENDENT → VENUE_DEPENDENT → MIXED (favorable-but-unstable conflict) → HISTORICALLY_FAVORABLE (≥ 0.65) → HISTORICALLY_UNFAVORABLE (≤ 0.35) → MIXED (middle band). Every decision carries reconstructible reasons.
- **Ranking / Explanation / Research / Feedback (`ranking.ts`, `explanation.ts`, `research-context.ts`, `feedback.ts`)** — per-domain deterministic ranking (score desc → candidateId asc; INSUFFICIENT_EVIDENCE/UNKNOWN/NOT_COMPARABLE excluded with explicit reasons; every profile ranked or excluded); reconstructible explanations (dimension-by-dimension contributions, strong/weak dimensions, sufficiency, dependencies, leakage and stability effects, missing evidence, classification rationale — no opaque scores, no certainty claims, no authority language); structured research context (identity, similar observations, evidence summary, score components, dependency signals, gaps, recommended informational questions); decision feedback records plus immutable outcome reconciliation (FAVORABLE_BUT_NEGATIVE / UNFAVORABLE_BUT_POSITIVE drift observations — history is never rewritten).
- **Replay, Audit, Invariants (`replay.ts`, `audit.ts`, `invariants.ts`, `engine.ts`)** — byte-identical canonical-JSON replay; hash-chained `oship.opportunity-intelligence.v1` audit over exactly 13 event types (GENESIS-rooted; tampering, reordering, payload substitution, truncation and extension fail verification); **66 hard fail-closed invariants** — determinism (stage rebuilds byte-identical), identity, domain isolation, AFIS/ABL semantics, evidence states, score bounds/decomposition/weight normalization/no-magic-weights, classification consistency, dependency rule, leakage counted once, stability explicitness, distribution historical-only, ranking coverage, explanation reconstructibility, NO_FUTURE_CERTAINTY_CLAIMS and NO_AUTHORITY_LANGUAGE vocabulary scans, research informational-only, feedback mirroring, reconciliation non-destructive, audit integrity, fail-closed rejections, no-silent-repairs, informational-only and no execution/treasury/credential surface.

### Demo (`opportunity-intelligence-demo.ts`)
`demo:opportunity-intelligence` drives the real engine through 48 assertion-backed sections — historical input, candidate intake, the rejection gallery, AFIS and ABL semantics, cross-domain isolation, similarity (pool/components/floor/top-K/ordering), learned features (class/strategy/venue), regime match, strategy and venue history, leakage (apparent vs realized vs adjusted, counted once), evidence (counts/freshness/consistency/confidence spread), stability, distribution (bands/quartiles/realization), score (dimensions/weights/decomposition/nulls/disclaimer), dependencies, classification (precedence/bands), profile, explanation (reconstructible/no certainty claims), research context, feedback, reconciliation, rankings (AFIS/ABL exclusions), audit (chain/tamper resistance), replay, invariants, security — ending `SYSTEM STATUS: RECONCILED`.

## Sprint 039 — Unified Decision Intelligence & Counterfactual Evaluation Engine (`services/market-engine/src/intelligence/decision/`)

**THE DECISION INTELLIGENCE ENGINE IS AN ANALYTICAL DECISION-SUPPORT LAYER, NOT AN EXECUTION ENGINE, NOT A CAPITAL ALLOCATOR AND NOT AN ORACLE.** It turns a validated Sprint 037 learning result, a Sprint 038 opportunity-intelligence profile and a set of hypothetical candidate alternatives into an evidence-bound, associational comparison with an explicit recommendation — strictly upstream of Strategy and AEGIS. Counterfactual means "What does the existing evidence say about this alternative if this alternative had been selected?" — never "What will happen in the future?"

Lifecycle (one `DecisionIntelligenceEngine.analyze()`): **Opportunity → Opportunity Intelligence → Decision Context → Candidate Alternatives → Compatibility → Counterfactual Evaluation → Trade-off Analysis → Dominance → Evidence-Bound Recommendation → Explanation → Research / Strategy Input**.

- **Decision Context (`context.ts`, `types.ts`)** — canonical, serializable decision frame: identity (`dctx_`-prefixed, content-derived), base candidate, domain, class, strategy, venues, engine version, configuration fingerprint, `informational: true`, `ASSOCIATIONAL_ONLY`.
- **Alternatives & Validation (`alternative.ts`, `alternative-builder.ts`)** — `AlternativeSpec` with explicit nulls meaning "no override"; an auto-constructed BASELINE per decision (its id is reserved — duplicate submissions are rejected); fail-closed validation over twelve rejection codes (malformed, missing identity, unknown kind, duplicate id, base mismatch, cross-domain strategy, unknown venue, wrong side semantics, non-canonical order, empty variation, incompatible units, ambiguous semantics incl. odds on AFIS legs); per-domain standard builder sets (AFIS strategy/venue/execution; ABL orientation/market/venue — never cross-domain).
- **Compatibility (`compatibility.ts`)** — ten-plus deterministic checks (domain, class, identity, semantics, units, ordering) gate every alternative before any evaluation; the baseline is always assessed and audited.
- **Counterfactual Evaluation (`counterfactual.ts`)** — explicit override application produces the hypothetical candidate; the Sprint 038 opportunity profile is evaluated for it; evidence gaps and conflicts are named per alternative; `counterfactualOnly: true`.
- **Historical Comparison (`historical-comparison.ts`)** — every accepted pair compared descriptively (means, medians, dispersion, deltas); `descriptiveOnly` on every pair; cross-domain pairs are never comparable.
- **Dependency Axes (`regime-analysis.ts`, `strategy-analysis.ts`, `venue-analysis.ts`, `leakage.ts`, `stability.ts`, `evidence.ts`)** — six axes surfaced per alternative; dependencies (regime/strategy/venue) are detected via cohort spreads with all groups exposed and are never silently generalized; leakage is counted exactly once (adjusted = realized + burden); stability informs as an explicit trade-off dimension, never a silent override.
- **Trade-off Score (`tradeoff.ts`)** — twelve explicit dimensions with config-driven auditable weights, availability renormalization (effective weights sum to 1), bitwise-exact decomposition, null scores when evidence confidence is INSUFFICIENT/NOT_COMPARABLE/CONFLICTED.
- **Dominance & Classification (`dominance.ts`, `classification.ts`)** — nine states (DOMINANT_BY_EVIDENCE, WEAKLY_PREFERRED, NO_DOMINANT_OPTION, INSUFFICIENT_EVIDENCE, CONFLICTED, VENUE_DEPENDENT, REGIME_DEPENDENT, STRATEGY_DEPENDENT, NOT_COMPARABLE) under configurable dominant/weak/tie bands; DOMINANT_BY_EVIDENCE means "better supported by existing evidence" and its reasons always state that support is not certainty.
- **Ranking / Recommendation (`ranking.ts`, `recommendation.ts`)** — deterministic ordering (score descending, canonical id tie-breaks) with explicit exclusions and reasons; the recommendation carries the exact disclaimer, selects nothing under CONFLICTED/INSUFFICIENT_EVIDENCE, exposes dependencyState and a twelve-row trade-off breakdown.
- **Scenario Matrix (`scenario.ts`)** — alternative × evidence × regime × strategy × venue cells backed only by real observation ids; unsupported combinations are explicitly INCOMPATIBLE, never inferred.
- **Explanation / Research / Feedback (`explanation.ts`, `research-context.ts`, `feedback.ts`)** — reconstructible explanations (summary, per-alternative rationales, evidence for/against, axis effects, conflict conditions, acceptance decisions, boundary statements); informational research contexts with prioritized questions; feedback records plus immutable outcome divergence reconciliation (AGREEMENT / RECOMMENDED_BUT_NEGATIVE / SPURNED_ALTERNATIVE_POSITIVE / INSUFFICIENT_RECOMMENDATION / UNDETERMINABLE, regime/strategy/venue/leakage mismatch flags — drift observations, never history rewrites).
- **Replay, Audit, Invariants, Engine (`replay.ts`, `audit.ts`, `invariants.ts`, `ids.ts`, `engine.ts`)** — canonical-JSON byte-identical replay (internal double-run, byte-compared); hash-chained `oship.decision-intelligence.v1` audit over exactly 14 event types (tamper/reorder/substitution/truncation/extension fail closed); 83 hard fail-closed invariants (determinism, isolation, no-execution-authority, no-future-certainty, no-authority-language, audit-covers-lifecycle, spec coverage, reproducible ranking, disclaimer presence, informational flags, credential-surface absence); the engine fails closed on unvalidated learning results and rejected base candidates.

### Demo (`decision-intelligence-demo.ts`)
`demo:decision-intelligence` drives the real engine through 48 assertion-backed sections — decision context, AFIS/ABL opportunities, candidate alternatives, compatibility, the rejection gallery, historical comparison, evidence quality, the six dependency axes, scenario matrix, trade-off decomposition, dominance states across six runs, recommendations with the exact disclaimer, conflict/insufficient-evidence handling, regime/strategy/venue-dependent cases, AFIS and ABL isolation, BACK/LAY preservation, cross-domain rejection, explanation, research context, feedback and divergence, replay, audit tamper detection, deterministic triple runs, fail-closed, edge cases, serialization with no fabricated keys, evidence gaps, ties, ranking, recommendation/audit reconstruction, full lifecycle, security boundary, no-certainty guard, the alternative-builder surface, counterfactual determinism and the final reconciled state — ending `SYSTEM STATUS: RECONCILED`.

## Sprint 040 — Unified Decision Governance & Strategy Handoff Engine (`services/market-engine/src/intelligence/governance/`)

**THE GOVERNANCE ENGINE IS A GOVERNANCE AND HANDOFF LAYER, NOT A STRATEGY ENGINE, NOT AN EXECUTION ENGINE, NOT A CAPITAL ALLOCATOR AND NOT AN ORACLE.** It answers one question over a validated Sprint 039 decision-intelligence result: "Is this decision result sufficiently valid, comparable, fresh, stable and policy-compliant to hand to Strategy, and what information and limitations must accompany the handoff?" It sits inside Intelligence, strictly between Decision Intelligence and the existing Strategy layer — Human → AETHER → OSHIP Core → Intelligence → Strategy → AEGIS → Treasury → Execution — and never bypasses, duplicates or overrides Strategy Registry, AEGIS, Treasury, Execution, Portfolio, Risk or Allocation.

Lifecycle (one `GovernanceEngine.govern()`): **Decision Result → Governance Context → Policy Validation → Evidence Gate → Safety Gate → Comparability Gate → Freshness Gate → Stability Gate → Dependency Gate → Handoff Classification → Strategy Handoff Package → Research / Feedback**.

- **Governance Context (`context.ts`, `types.ts`)** — canonical, immutable, informational frame mirroring the Sprint 039 states verbatim (recommendation/dominance/evidence/confidence, unresolved conflicts, regime/strategy/venue dependencies, leakage, stability, freshness, sample adequacy, comparability, research gaps, feedback state, versions); `validateDecisionResult` fails closed on upstream invariant/replay violations, non-finite payloads, fabricated keys and domain-semantic violations (odds on AFIS legs → `INVALID_AFIS_SEMANTICS`; BUY/SELL on ABL legs → `INVALID_BACK_LAY_SEMANTICS`; odds ≤ 1 or missing market/selection identity → `INVALID_ABL_SEMANTICS`).
- **Twelve Explicit Policies (`policy.ts`, `policy-registry.ts`)** — evidence sufficiency, stale evidence, conflicted evidence, comparability, stability, leakage, regime/strategy/venue dependency, research gaps, semantic safety, authority boundaries; every policy is versioned, validated and audited per run; rejections are never hidden behind a generic "not recommended".
- **Evidence Gate (`evidence-gate.ts`)** — PASS / PASS_WITH_LIMITATIONS / BLOCK_INSUFFICIENT_EVIDENCE / BLOCK_CONFLICTED / BLOCK_STALE / BLOCK_NOT_COMPARABLE with deterministic precedence NOT_COMPARABLE → CONFLICTED → INSUFFICIENT → STALE → LIMITED → PASS.
- **Safety Gate (`safety-gate.ts`)** — rejects probability/forecast/expected-profit/ROI/guaranteed-return/certainty claims and execution instructions in requester annotations, fabricated output keys anywhere, and certainty language in narratives (negation-aware via `containsCertaintyClaim` — "support is not certainty" stays legal); the disclaimer is verbatim and excluded from scans.
- **Comparability Gate (`comparability-gate.ts`)** — AFIS BUY/SELL and ABL BACK/LAY are semantically distinct; raw AFIS↔ABL is never comparable (`rawDomainsComparable` false); cross-domain comparison requires an explicit `cross-domain.normalized.v1` normalization with total side mapping onto a neutral namespace, declared semantic loss, version match and policy attestation — never inferred; domain-spanning alternative sets are rejected before any gate runs.
- **Freshness / Stability / Dependency Gates (`freshness-gate.ts`, `stability-gate.ts`, `dependency-gate.ts`)** — FRESH/AGING/STALE/UNKNOWN with worst-of aggregation (STALE blocks unless `allowStaleAnalyticalOnly`; UNKNOWN never silently FRESH, blocks unless `allowUnknownFreshnessAnalyticalOnly`); STABLE/MODERATELY_STABLE/UNSTABLE/INSUFFICIENT never converted to probabilities (UNSTABLE blocks under `unstableBlocksHandoff`); INDEPENDENT/REGIME_/STRATEGY_/VENUE_/MULTI_DEPENDENT/UNKNOWN preserved into the package, fail-closed on unresolvable flags.
- **Authority Check (`authority-check.ts`)** — requester annotations carry no authority verbs, no command/credential keys anywhere, the consumed result stays informational and the recommendation names at most an alternative id; seven protected authorities (Strategy Registry, AEGIS, Treasury, Execution, Portfolio, Risk, Allocation); violations are auditable in-band blocks (`AUTHORITY_BYPASS`), never silently executed.
- **Handoff Classification (`handoff-classification.ts`)** — eight states (HANDOFF_ALLOWED, HANDOFF_ALLOWED_WITH_LIMITATIONS, HANDOFF_REQUIRES_RESEARCH, HANDOFF_BLOCKED, HANDOFF_NOT_COMPARABLE, HANDOFF_CONFLICTED, HANDOFF_STALE, HANDOFF_INSUFFICIENT_EVIDENCE) under deterministic precedence unsafe → authority-bypass → dependency-unknown → not-comparable → conflicted → insufficient → unknown-freshness → stale → unstable-blocked → research → limitations → allowed; blocked results are never silently converted into alternative recommendations (`recommendedAlternativeId: null`).
- **Restrictions (`handoff-restrictions.ts`)** — thirteen machine-readable codes (ANALYTICAL_ONLY, NO_EXECUTION, RESEARCH_REQUIRED, LIMITED_TO_DOMAIN/VENUE/STRATEGY, REGIME_SPECIFIC, STALE_EVIDENCE_WARNING, INSUFFICIENT_SAMPLE_WARNING, STABILITY_WARNING, AGING_EVIDENCE_WARNING, NORMALIZED_COMPARISON_ONLY, LEAKAGE_WARNING) with canonical ordering, scopes and validation; every handoff carries the analytical-only baseline.
- **Strategy Handoff Package & Strategy Input (`handoff.ts`, `strategy-input.ts`)** — deterministic, immutable, informational packages (`decision-governance.handoff.v1`) carrying opportunity identity, domain, recommendation, alternative ranking, evidence summary, trade-offs, dominance, evidence limitations, dependencies, leakage/stability/freshness/comparability status, research requirements, governance result and restrictions, source versions, the verbatim disclaimer and an audit identity; the minimal strategy-input view (`decision-governance.strategy-input.v1`) declares `strategyDecides: true`; packages contain no executable orders, execution instructions, Treasury/capital-allocation commands, AEGIS authorization, provider or exchange/bookmaker credentials.
- **Research & Feedback (`research-context.ts`, `feedback.ts`)** — seven escalation kinds (RESEARCH_REQUIRED, EVIDENCE_REFRESH_REQUIRED, REGIME_/STRATEGY_/VENUE_COVERAGE_REQUIRED, COMPARABILITY_REQUIRED, LEAKAGE_INVESTIGATION_REQUIRED) flow to the existing Research Plane (no second Research authority); structured feedback records (GOVERNANCE_BLOCKED_DECISION, GOVERNANCE_ALLOWED_WITH_LIMITATIONS, EVIDENCE_GAP_FEEDBACK, STALE_EVIDENCE_FEEDBACK, CONFLICT_FEEDBACK, DEPENDENCY_DETECTED_FEEDBACK, RECOMMENDATION_WEAKENED/PRESERVED) flow to the existing Learning/Feedback architecture (no new Learning authority).
- **Replay, Audit, Invariants, Engine (`replay.ts`, `audit.ts`, `invariants.ts`, `ids.ts`, `engine.ts`)** — canonical-JSON byte-identical replay (internal double-run, byte-compared, key-order independent); hash-chained `oship.decision-governance.v1` audit over exactly 16 event types (tamper/reorder/substitution/truncation/extension/foreign-event fail closed); **77 hard fail-closed invariants** (authority non-duplication ×7, semantic safety, AFIS/ABL semantics, honesty of freshness/stability/leakage, determinism, audit integrity, immutability, strategy boundary, no credential/command propagation, blocked-never-recommends); content-derived prefixed ids (`gov_`, `gctx_`, `gevg_`, `gcls_`, `gres_`, `ghof_`, `gstr_`, `gea_`, …) with no timestamp/random/PID dependence; a 22-entry rejection gallery over explicit codes (`INVALID_GOVERNANCE_CONTEXT`, `INVALID_DECISION_RESULT`, `MISSING_DECISION_ID`, `MISSING_OPPORTUNITY_ID`, `MISSING_DOMAIN`, `UNSUPPORTED_DOMAIN`, `NONDETERMINISTIC_INPUT`, `INVALID_AFIS_SEMANTICS`, `INVALID_ABL_SEMANTICS`, `INVALID_BACK_LAY_SEMANTICS`, `LEAKAGE_INCONSISTENCY`, `STABILITY_INCONSISTENCY`, `INVALID_RESTRICTION`, …).

### Demo (`decision-governance-demo.ts`)
`demo:decision-governance` drives the real engine through 54 assertion-backed sections — governance context, the Sprint 039 state mirror, the twelve-policy registry and its validation, evidence-gate outcomes and precedence, safety-gate scans (predictions, certainty with negation tolerance, execution instructions, fabricated keys), the verbatim disclaimer, comparability and BACK/LAY preservation, normalization valid and invalid variants, freshness states and policies, stability states and policies, dependency states, authority boundaries, all eight handoff classifications, restriction machinery, the LIQ/AFIS/ABL handoff packages, package boundaries (no orders/credentials/commands), audit identity, the strategy input and its blocked-null behavior, all seven research escalations, the research context rebuild, feedback records, byte-identical replay and key-order permutation, the audit chain and its five violation modes, the audit log class, the 77 invariants and their coverage, configuration, deterministic ids, engine determinism, the full fail-closed rejection gallery, malformed inputs, upstream integrity, AFIS and ABL domain validation, cross-domain isolation, the clean handoff, corpus outcomes, immutability and the final boundary summary — ending `SYSTEM STATUS: RECONCILED`.

## Sprint 041 — Unified Strategy Decision Synthesis & AEGIS-Ready Intent Engine (`services/market-engine/src/intelligence/strategy-intent/`)

**THE INTENT ENGINE IS AN INTENT SYNTHESIS LAYER, NOT A STRATEGY ENGINE, NOT AN EXECUTION ENGINE, NOT A CAPITAL ALLOCATOR AND NOT AN ORACLE.** It answers one question over a governed Sprint 040 handoff: "Given a governed Decision Intelligence handoff, what structured strategic intent should the existing Strategy authority receive?" It sits inside Intelligence, strictly between Decision Governance and the existing Strategy layer — Decision Intelligence → Decision Governance → Strategy Intent → Existing Strategy → AEGIS → Treasury → Execution — and never creates a parallel Strategy authority, never mutates the Strategy Registry, and never touches AEGIS, Treasury, Execution, Portfolio, Risk or Allocation.

Lifecycle (one `StrategyIntentEngine.synthesize()`): **Source Validation → Intent Context → Governance/Decision Facts → Classification → Priority → Objective → Alternatives → Ranking → Dependencies → Restrictions → Research → Evidence → Explanation → Feedback → Strategy Boundary → Audit → Invariants → Replay**.

- **Source Validation (`source-validation.ts`)** — consumes ONLY governed Sprint 040 outputs: frozen governance and decision results, verified `oship.decision-governance.v1` audit chains, bound audit-identity heads, invariant/replay flags, annotation semantic scans (PREDICTIVE/EXECUTION/TREASURY/AEGIS/UNSAFE semantics) and provenance pairing — a decision from another governance run is an orphan and rejects with `INVALID_PROVENANCE`.
- **Intent Context (`context.ts`)** — immutable mirror of the governed identity: decision, governance, opportunity, domain, classification, recommendation status, evidence state, stability/freshness/comparability/dependency states, research gaps and versions; nothing re-derived.
- **Classification (`classification.ts`)** — eight states mapped 1:1 from the eight handoff classifications with NO silent fallback: blocked families surface `preferredAlternativeId: null` and an empty acceptable set; a classification must be supported by its own evidence gates (allowed-with-conflicted-evidence rejects `CONFLICTED_EVIDENCE`, insufficient-without-basis rejects `INSUFFICIENT_EVIDENCE`, stale-without-stale-status rejects `STALE_EVIDENCE`, not-comparable-without-status rejects `NOT_COMPARABLE`).
- **Priority & Objective (`priority.ts`, `objective.ts`)** — six analytical priority levels ordering governance attention and research (never probability, return or urgency); six analytical objective classes (PRESERVE_EVIDENCE_SUPPORTED_EDGE, PREFER_STABLE_ALTERNATIVE, PREFER_HISTORICALLY_SUPPORTED_ALTERNATIVE, MINIMIZE_EVIDENCE_CONFLICT, REQUIRE_MORE_RESEARCH, NO_ACTIONABLE_INTENT).
- **Alternatives & Ranking (`alternatives.ts`, `ranking.ts`)** — canonical ids, semantic identity (AFIS BUY/SELL with no betting fields; ABL BACK/LAY + market/selection identity + decimal odds > 1 — never collapsed, never converted), evidence state, compatibility and limitations preserved with roles PREFERRED/SECONDARY/REJECTED/UNSUPPORTED; deterministic ordering; the acceptable list is gated behind classification actionability.
- **Dependencies & Restrictions (`dependencies.ts`, `restrictions.ts`)** — six dependency states preserved from governance (never removed because a preferred alternative exists, contradictions fail closed); seventeen restriction codes with the informational baseline (ANALYTICAL_ONLY, NO_EXECUTION, NO_TREASURY_ACTION, NO_AEGIS_AUTHORIZATION) on EVERY intent plus governance-mapped and intent-derived codes, canonically ordered, surviving serialization and replay.
- **Research, Evidence, Explanation, Feedback (`research.ts`, `evidence.ts`, `explanation.ts`, `feedback.ts`)** — eight escalation classes routed to the existing Research Plane with explicit provenance (GOVERNANCE_ESCALATION vs INTENT_DERIVED); evidence-backed rationale with explicitly historical support; deterministic explanations pinning source decision and governance ids; eight feedback kinds to the existing Learning/Feedback architecture (no new authorities).
- **Strategy Boundary (`strategy-boundary.ts`)** — `strategyDecides: true`, informational-only: no order quantity/price, venue command, API request, credential, signing material, Treasury transfer, capital allocation, AEGIS authorization or bookmaker/exchange submission is possible; quoted requester speech inside governance reasons is inert reported text; violations fail closed with `STRATEGY_BOUNDARY_VIOLATION`.
- **Replay, Audit, Invariants, Engine (`replay.ts`, `audit.ts`, `invariants.ts`, `ids.ts`, `engine.ts`)** — canonical key-order-independent byte-identical serialization; replay (internal double-run, permuted-key, replay-from-serialized with tamper detection); hash-chained `oship.strategy-intent.v1` audit over 16 event types (tamper/reorder/substitution/truncation/extension/foreign-event fail closed); **93 hard fail-closed invariants** per run; content-derived prefixed ids (`sint_`, `sctx_`, `sprv_`, `sres_`, `srsc_`, `sfdb_`, `sea_`, `sfp2_`, …) with zero timestamp/random/PID/insertion-order dependence; a 57-entry rejection gallery over 24 distinct codes of the 25-code vocabulary.

### Demo (`strategy-intent-demo.ts`)
`demo:strategy-intent` drives the real engine through 60 assertion-backed sections — engine identity, the governed input contract, fail-closed source validation, annotation safety scans, the intent context, mirrored statuses, the eight classifications, no-silent-fallback, priorities, objectives, alternative sets, AFIS and ABL semantics, never-collapsed sides, unsupported alternatives, blocked-family gating, no-dominant secondaries, ranking determinism, dependency preservation, the restriction vocabulary and baseline, governance restriction mapping, restriction replay survival, research classes and provenance, the evidence bundle, semantic limitations, the verbatim disclaimer, no predictive semantics, no execution leakage, the strategy boundary, protected authorities, explanations, feedback, provenance, source versions, deterministic ids, canonical serialization, replay, the audit schema and tamper detection, the audit identity, the invariant battery, immutability, configuration, cross-domain isolation, explicit normalization, classification/evidence consistency, the 57-entry rejection gallery, quoted-speech inertness, full corpus coverage, id distinctness and the final reconciled state — ending `SYSTEM STATUS: RECONCILED`.

## Sprint 042 — Unified Strategy-Intent Evaluation & Portfolio Decision Bridge (`services/market-engine/src/intelligence/strategy-intent-evaluation/`)

**THE EVALUATION ENGINE IS AN ANALYTICAL DECISION BRIDGE, NOT A PORTFOLIO ENGINE, NOT A RISK ENGINE, NOT AN ALLOCATOR, NOT A STRATEGY REGISTRY, NOT AN EXECUTION PLANNER AND NOT AN ORACLE.** It answers one question over an immutable Sprint 041 `StrategyIntent`: "Is this intent sufficiently supported, constrained, comparable, fresh, stable and structurally valid to be CONSIDERED by the existing Portfolio/Risk/Allocation decision plane?" It sits inside Intelligence, strictly between Strategy Intent and the existing downstream decision plane — Decision Intelligence → Decision Governance → Strategy Intent → Strategy Intent Evaluation → Existing Portfolio/Risk/Allocation — and never creates a parallel decision authority, never allocates, sizes, reserves or approves anything. **Eligibility NEVER means approved for trading, betting, execution, capital allocation or strategy activation.**

Lifecycle (one `StrategyIntentEvaluationEngine.evaluate()`): **Source Validation → Intent Integrity → Evidence → Safety → Comparability → Freshness → Stability → Dependency → Restriction Analysis → Portfolio-Interface Compatibility → Evaluation Dimensions → Classification → Downstream Eligibility → Research/Feedback Escalation → Explanation → Boundary → Audit → Invariants → Replay**.

- **Source Validation (`source-validation.ts`)** — consumes ONLY governed Sprint 041 outputs: frozen results, verified `oship.strategy-intent.v1` audit chains (with the replay-seal tolerance), bound audit identities, invariant/replay flags, complete provenance chains with pinned upstream versions, and a **recomputed whole-result fingerprint** — a result mutated after sealing (forged classification, edited restriction list, swapped ranking) rejects with `INVALID_INTENT_SOURCE`.
- **Intent Integrity (`intent-integrity.ts`)** — structural fail-closed consistency: classification ↔ evidence/freshness/comparability agreement, the informational baseline present, unique known restriction codes, classification-cause restrictions, `NORMALIZED_COMPARISON_ONLY` ↔ `COMPARABLE_VIA_NORMALIZATION` agreement, dependency state ↔ flag agreement, and actionable intents must carry historical support and a preferred alternative.
- **Gates (`gates.ts`)** — evidence/comparability/freshness/stability/dependency gates over the governed intent states; single-domain enforcement with AFIS-no-odds and ABL-odds>1 + BACK/LAY semantics — one intent never spans both domains, and no alternative mixes or converts BUY/SELL with BACK/LAY.
- **Safety (`safety-validation.ts`)** — ten semantic vocabularies (prediction, future value, execution, treasury, portfolio, risk, allocation, strategy boundary, authority, policy) with clause-scoped negation and quoted-span stripping over requester annotations and evaluation narratives; boundary-continuation phrases ("Aegis stays the execution authorization author") are inert.
- **Restriction Analysis (`restriction-analysis.ts`)** — every Sprint 041 restriction carried verbatim (never weakened) plus one derived `DOWNSTREAM_CONSIDERATION_ONLY` boundary code; canonical order, unique codes, non-empty reasons.
- **Portfolio Interface (`portfolio-interface.ts`)** — nine protected downstream authorities (Portfolio, Risk, Allocation, Strategy Registry, AEGIS, Treasury, Execution, Research Plane, Learning/Feedback); forbidden-key and execution-verb scans; the four-check boundary (no command keys, no execution narrative, informational, downstream-decides) — violations throw `PORTFOLIO_BOUNDARY_VIOLATION`.
- **Dimensions, Classification, Eligibility (`dimensions.ts`, `classification.ts`, `eligibility.ts`)** — exactly **18 explicit dimensions** (no hidden scoring, no ML); **13 deterministic classifications** (EVALUATION_ALLOWED, _ALLOWED_WITH_LIMITATIONS, _REQUIRES_RESEARCH, _BLOCKED, _INSUFFICIENT_EVIDENCE, _NOT_COMPARABLE, _CONFLICTED, _STALE, _UNSTABLE, _STRATEGY_DEPENDENT, _VENUE_DEPENDENT, _REGIME_DEPENDENT, _MIXED) with fixed precedence — blocked sources are never un-blocked, deficient gates demote actionable intents, conflicting gates fail closed; the frozen map to **9 downstream eligibility states** (ELIGIBLE_FOR_CONSIDERATION, ELIGIBLE_WITH_RESTRICTIONS, RESEARCH_REQUIRED, BLOCKED, NOT_COMPARABLE, INSUFFICIENT_EVIDENCE, STALE, UNSTABLE, CONFLICTED); blocked families surface no alternatives, dependency families map to RESEARCH_REQUIRED.
- **Research, Feedback, Explanation, Provenance (`research.ts`, `feedback.ts`, `explanation.ts`, `provenance.ts`)** — Sprint 041 research requirements carried verbatim with explicit provenance plus evaluation-derived classes (MIXED escalates regime+strategy+venue research); eight feedback kinds to the existing Learning/Feedback architecture; explanations pinning source ids with declared semantic limitations; provenance Opportunity → Decision → Governance → Strategy Intent → Evaluation with every engine version pinned.
- **Replay, Audit, Invariants, Engine (`replay.ts`, `audit.ts`, `invariants.ts`, `ids.ts`, `engine.ts`)** — canonical key-order-independent byte-identical serialization; internal double-run byte-compare (result AND audit chain); hash-chained `oship.strategy-intent-evaluation.v1` audit over 19 event types (tamper/reorder/substitution/truncation/extension/foreign/provenance/classification/eligibility all fail verification, with audit-binding checks); **86 hard fail-closed invariants** per run; content-derived timestamp-free prefixed ids (`eval_`, `evctx_`, `evdim_`, `evres_`, `evrsc_`, `evfdb_`, `evprv_`, `evexp_`, `evbnd_`, `evea_`, `evfp_`, `evcfg_`); a **70-entry rejection gallery over 29 distinct codes** (INVALID_EVALUATION_CONTEXT … AUDIT_INTEGRITY_FAILURE).

### Demo (`strategy-intent-evaluation-demo.ts`)
`demo:strategy-intent-evaluation` drives the real engine through 80 assertion-backed sections — engine identity, the decision-bridge question, the thirteen classifications, the nine eligibility states, the eighteen dimensions, the eight gates, governed-source discipline, the sealed intent fingerprint, the AFIS informational ladder (clean/restricted/aging/normalized/conflicted/blocked), insufficient/stale/unstable families, the dependency families, ABL BACK/LAY preservation, market/selection identity, decimal odds, cross-domain rejection, invalid normalization, missing provenance/dependency, authority and semantic-safety violations, replay determinism, audit tamper/reorder/substitution/truncation/extension/foreign/binding detection, forbidden keys, the nine protected authorities, the invariant battery, the full corpus sweep, the rejection gallery and the final reconciled state — ending `SYSTEM STATUS: RECONCILED`.
